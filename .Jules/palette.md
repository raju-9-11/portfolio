...
## 2025-02-28 - Semantic Progress Bars
**Learning:** Visual progress bars implemented with `div`s are invisible to screen readers and often cause console warnings when props are passed to the DOM.
**Action:** Always use `role="progressbar"` with `aria-valuenow`/`aria-labelledby`, and use transient props (prefixed with `$`) in styled-components for dynamic styling values.
