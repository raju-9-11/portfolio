...
## 2025-02-27 - Semantic Progress Bars
**Learning:** Progress bars implemented as `div`s are invisible to screen readers. Using `role="progressbar"` with `aria-valuenow` and `aria-labelledby` makes them accessible without changing visual design.
**Action:** Audit all visual progress indicators (like in BootSequence) and add ARIA roles and labels.
